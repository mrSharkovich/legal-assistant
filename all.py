...
def magic_function(file_path, selected_tags):
    ...
    res = res + file_path + selected_tags_string
    return res
...
class Ui_AgreementWindow(object):
    ...
    def get_privacy_text(self):
        privacy_text = ""
        with open("docs/for_users/privacy.txt", "r", encoding="utf-8", errors='ignore') as file:
            privacy_text = file.read()
        return privacy_text

    def get_agreement_text(self):
        agreement_text = ""
        with open("docs/for_users/agreement.txt", "r", encoding="utf-8", errors='ignore') as file:
